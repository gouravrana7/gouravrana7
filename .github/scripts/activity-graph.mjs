...
const W = 840, H = 260, PAD_L = 46, PAD_R = 20, PAD_T = 56, PAD_B = 40;
...
function toPoints(days) {
  ...
  return days.map((d, i) => ({
    ...
    y: PAD_T + (1 - d.contributionCount / max) * (H - PAD_T - PAD_B),
    ...d,
  }));
}
...
function render(user, days) {
  ...
  <text x="${PAD_L}" y="32" fill="${TITLE}" font-size="18" font-weight="600">${user}'s Contribution Graph</text>
  <text x="${W - PAD_R}" y="32" fill="${AXIS}" font-size="12" text-anchor="end">last 31 days</text>
...
function demo() {
  ...
  const pts = toPoints(days);
  console.assert(pts.length === 31, 'expected 31 points');
  console.assert(pts.every((p, i) => i === 0 || p.x > pts[i - 1].x), 'x must increase');
  console.assert(pts.every((p) => p.y >= PAD_T - 0.01 && p.y <= H - PAD_B + 0.01), 'y within plot area');
  console.assert(pts.find((p) => p.contributionCount === 6).y === PAD_T, 'max maps to plot top');
  console.assert(pts.find((p) => p.contributionCount === 0).y === H - PAD_B, 'zero maps to baseline');

  const svg = render('demo', days);
  console.assert(svg.startsWith('<svg') && svg.includes('</svg>'), 'svg well-formed');
  console.assert((svg.match(/<circle/g) ?? []).length === 31, 'one dot per day');

  // Flat history must not divide by zero.
  const flat = render('demo', days.map((d) => ({ ...d, contributionCount: 0 })));
  console.assert(!flat.includes('NaN'), 'flat history produced NaN');
...
if (arg1 === '--demo') {
  ...
  writeFileSync(out, render(arg1, await fetchDays(arg1, token)));
